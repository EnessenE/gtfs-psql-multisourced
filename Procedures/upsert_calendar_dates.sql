...
AS $$
BEGIN
    -- Perform a single bulk insert for all rows in the _calendar_dates array
    ...
    SELECT DISTINCT
        ...
        _calendar_date.date, 
        _calendar_date.exception_type_data,  -- Use _exception_type_data from the array
    ...
    ON CONFLICT (data_origin, service_id, date)  -- Specify the unique constraint
