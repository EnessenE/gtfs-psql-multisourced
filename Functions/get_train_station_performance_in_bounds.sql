DROP FUNCTION IF EXISTS public.get_train_station_performance_in_bounds(double precision, double precision, double precision, double precision, integer);

CREATE OR REPLACE FUNCTION public.get_train_station_performance_in_bounds(
...
-- recent_updates joins against the already-filtered train_stations set,
-- avoiding a full re-scan of trip_updates_stop_times with lat/lon predicates.
recent_updates AS (
    SELECT
        rs.primary_stop,
        tust.data_origin,
    ...
    INNER JOIN public.related_stops rs
        ON rs.primary_stop = ts.primary_stop_id
    INNER JOIN public.stops s
        ON s.internal_id = rs.related_stop
       AND s.data_origin = rs.related_data_origin
    INNER JOIN public.trip_updates_stop_times tust
        ON tust.stop_id = s.id
       AND tust.data_origin = s.data_origin
),
aggregated AS (
    SELECT
        ru.primary_stop,
...
ALTER FUNCTION public.get_train_station_performance_in_bounds(double precision, double precision, double precision, double precision, integer)
    OWNER TO postgres;