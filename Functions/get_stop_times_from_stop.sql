...
SELECT
	...
	INNER JOIN calendar_dates ON trips.service_id = calendar_dates.service_id
		AND calendar_dates.data_origin = trips.data_origin AND ((calendar_dates.date::date >=from_date::date)
                )
    WHERE
...
(primary_stop = target)
	AND stop_type = target_stop_type
            ...
            AND((stop_times.arrival_time >= from_time::time
                    AND calendar_dates.date::date >= from_date::date)
                OR(calendar_dates.date::date > from_date::date ))
